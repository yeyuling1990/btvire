...
//  conf.set("spark.executor.memory", "2g")
  ...
  println("----------------------")
    ...
    val userCountRdd = topiccountdata.map(tup => (tup._1, tup._4)).reduceByKey(_ + _, 3).collect().foreach(tuple => println(tuple._1))
    println("**********************")
    var channelCount: scala.collection.mutable.Map[String, Int] = scala.collection.mutable.Map[String, Int]()
    sc.broadcast(channelCount)
    println("&&&&&&&&&&&&&&&&&&&&&&")

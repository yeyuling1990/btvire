...
public class VectorModel {
	...
	public float[] getVectorOfStr(String str) {
...
//			if(path.startsWith("hdfs:"))
			...
			if(path.startsWith("/tmp"))
			{
				...
				dis = new DataInputStream(new BufferedInputStream(fs.open(new Path(path))));
			}
			else
			{
	...
	public Set<WordScore> similar(String filterStartStr,  float[] center) {
		if (center == null || center.length != vectorSize) {
			return Collections.emptySet();
		}
		...
		for (Map.Entry<String, float[]> entry : wordMap.entrySet()) {
			...
			if(!sKey.startsWith(filterStartStr))
			{
				continue;
			}
			float[] vector = entry.getValue();
